...
import { Author, AUTHORS } from "./authors.ts";
...
function get_pretty_path(path: string) {
...
function get_authors_html(authors: Author[], allow_links: boolean) {
	return html.div({
		attributes: {
		...
		children: [
			...
			...authors.map((author) => {
				if (author.link && allow_links) {
					const attributes: {
						[key: string]:
							| string
							| string[]
							| html.StyleAttributeValue
							| null
							| undefined;
					} = {
						href: author.link,
					...
					if (author.theme) {
						attributes["ls_theme_color"] = author.theme;
					}
...
function get_times_html(times: Times) {
	...
	if (
		times.modification_time && times.creation_time !== times.modification_time
	) {
...
function get_article_metadata_html(
	authors: Author[],
	times: Times,
	allow_links: boolean,
) {
	return html.div({
		...
		children: [
			get_authors_html(authors, allow_links),
			get_times_html(times),
		],
	});
}

function get_tags_html(keywords: string[]) {
...
function visit_html_tree(node: html.Node, callback: (node: html.Node) => void) {
	callback(node);
...
async function process_blog_entry(
	path: string,
	pages_context: PagesContext,
	context: { root: string; authors: Record<string, Author> },
) {
	let title = get_pretty_path(path);

	let page_description: string | null = null;
	let embed_image: { url: string; alt: string } | undefined = undefined;
	let embed_video:
		...
		pages_context,
		{
			load_view: async function (_) {
				...
				const doc = await Deno.readTextFile(`${context.root}/${path}`)
					.then((content) =>
						md.parser.parse(content, create_common_markdown_parser_opts())
					);

				for (const block of doc.blocks) {
					if (block instanceof md.BlockCode && block.language) {
						await get_or_load_language(block.language.replace(/:apply$/, ""));
					}
				}

				article.children = md.render_to_html(
					doc,
					create_common_markdown_render_opts({
						block_code: {
							highlighter: (code, language, parent) => {
								if (Prism.languages[language]) {
									...
									parent.children =
										stuff.get_element_by_tag_name("code")!.children;
								} else {
				...
				process_property_from_html(article, "embed_image", (value) => {
					...
					if (!result) {
						throw new PageProcessError(
							path,
							`Could not parse properly embed image data "${value}"`,
						);
					}

					result[2] = result[2].substring(1, result[2].length - 1);
					if (!result[1].startsWith("http")) {
						result[1] = CONSTANTS.get_url(result[1]);
					}
				...
				process_property_from_html(article, "tag", (value) => {
					...
					tag_values.map((v) => v.trim().toLowerCase()).forEach((v) =>
						tags.push(v)
					);
				});
				...
				if (!page_description) {
					throw new PageProcessError(
						path,
						'Missing blog entry description, please add a comment starting with "description:".',
					);
				} else if (!times.creation_time) {
					throw new PageProcessError(
						path,
						'Missing blog entry creation time, please add a comment starting with "date:".',
					);
				}
				...
				article_metadata.append_child(
					html.create_element("h1").with_child(title),
				);
				article_metadata.append_child(
					get_article_metadata_html(authors, times as Times, true),
				);

				article.children.splice(0, 0, article_metadata);
				article.children.splice(1, 0, html.create_element("hr"));

				if (authors.length === 1 && authors[0].theme) {
					body.attr("ls_theme_color", authors[0].theme);
				}
				...
				return view.with_child(
					body
						.with_child(
							html.parse(
								/*html*/ `<sidenav id="main_nav" navigation_data="src/nav/main_nav.json">
									<header>
						...
						.with_child(
							html.parse(
								/*html*/ `<app_footer class="ls_sidenav_neighbor"></app_footer>`,
							),
						),
				);
			},
			load_script: (_) => {
				...
				let theme_color: string | undefined = undefined;

				if (authors.length === 1 && authors[0].name === "Layla") {
					theme_color = pages_context.app.themes.layla.primary;
				}

				let image_to_embed = embed_image;
				if (image_to_embed === undefined && authors.length === 1) {
					let url = authors[0].picture;

					if (url.startsWith("/")) {
						url = pages_context.app.get_url(url);
					}

					image_to_embed = {
						url,
						alt: `Profile picture of ${authors[0].name}.`,
					};
				}
				...
				return {
					...
					styles: [
						...
						{
							source:
								"https://cdn.jsdelivr.net/npm/katex@0.11.1/dist/katex.css",
							hash:
								"sha384-bsHo4/LA+lkZv61JspMDQB9QP1TtO4IgOf2yYS+J6VdAYLVyx1c3XKcsHh0Vy8Ws",
...
async function process_blog_index(
	entries: BlogEntry[],
	pages_context: PagesContext,
) {
	...
	return await process_page("/blog", pages_context, {
		load_view: function (_) {
			...
			entries.forEach((post) => {
				...
				const authors = metadata.page.custom!.authors as Author[];

				const post_description_doc = md.parser.parse(
				...
				const card = html.create_element("card")
					...
					.with_attr(
						"tags",
						(metadata.page.custom!.tags as string[]).join(", "),
					);

				if (authors.length === 1 && authors[0].theme) {
					card.attr("ls_theme_color", authors[0].theme);
				}

				const content = html.parse(/*html*/ `<card:content>
						<h2>${post.name}</h2>
						${
							get_article_metadata_html(
								authors,
								metadata.page.custom!.times as Times,
								false,
							).html()
						}
						${get_tags_html(metadata.page.custom!.tags as string[])}
					</card:content>`) as html.Element;
				card.append_child(content);
				content.append_child(
					md.render_to_html(
						post_description_doc,
						create_common_markdown_render_opts({
							block_code: {
								highlighter: (code, language, parent) => {
									if (Prism.languages[language]) {
...
const rss_tag: html.NamedTagData<html.Element> = {
	name: "rss",
...
const channel_tag: html.NamedTagData<html.Element> = {
	name: "channel",
...
const item_tag: html.NamedTagData<html.Element> = {
	name: "item",
	required_attributes: Object.freeze([]),
	self_closing: false,
	parse_inside: true,
...
async function generate_rss_feed(entries: BlogEntry[]) {
	const channel = channel_tag.create();
	const rss = rss_tag.create()
		.with_attr("version", "2.0")
		.with_child(channel);

	const link_tag: html.NamedTagData<html.Element> = {
		name: "link",
		required_attributes: Object.freeze([]),
		self_closing: false,
		parse_inside: true,
	...
	channel.with_child(
		html.create_element("title").with_child("LambdAurora's Blog"),
	)
		.with_child(
			html.create_element("description").with_child(
				"A personal and random blog!",
			),
		)
		.with_child(link_tag.create().with_child(CONSTANTS.get_url("/blog")))
		.with_child(html.create_element("language").with_child("en-us"))
		.with_child(
			html.create_element("pubDate").with_child(
				(entries[0].page.metadata.page.custom!.times as Times).creation_time
					.toUTCString(),
			),
		);
	...
	entries.forEach((post) => {
		channel.append_child(
			item_tag.create()
				...
				.with_child(
					html.create_element("guid").with_child(CONSTANTS.get_url(post.path)),
				)
				.with_child(link_tag.create().with_child(CONSTANTS.get_url(post.path)))
				.with_child(
					html.create_element("description").with_child(
						post.page.metadata.page.description,
					),
				)
				.with_child(
					html.create_element("pubDate").with_child(
						(post.page.metadata.page.custom!.times as Times).creation_time
							.toUTCString(),
					),
				),
		);
...
export async function process_all_blog_entries(
	pages_context: PagesContext,
	root = BLOG_ROOT,
) {
	const context = {
						...
						if (entry.isFile && entry.name.endsWith(".md")) {
							...
							await process_blog_entry(path, pages_context, context)
								.then(async function (page) {
									...
									const h1 = (page.content[1] as html.Element)
										.find_element_by_tag_name("h1");
									let name = get_pretty_path(path);
									...
									blog_entries.push({
										path: html_path,
										name: name,
										time: (metadata.page.custom!.times as Times).creation_time,
										page: page,
									});
								});
						}
